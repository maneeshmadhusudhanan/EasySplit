...
const UpdateFriend = () => {
  ...
  useEffect(() => {
    const fetchFriend = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/friend/getFriend/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const updatedFriend = { name, email, upi };

    try {
      const res = await fetch(`http://localhost:5000/api/friend/updateFriend/${id}`, {
        method: 'PUT',
    ...
    <div className='flex gap-[5%]'>
      <div className="w-[800px] h-[700px] mx-auto p-8 mt-5 border rounded-lg shadow-lg bg-gray-900 text-white">
        <header className="text-center mb-8">
          <h1 className="text-5xl font-bold">Update Friend</h1>
        </header>
